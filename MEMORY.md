...
- 제품명이 상세페이지 표기와 어긋나면 여전히 no_answer다("대웅징코샷" vs "대웅징코샷정240mg" 이력) — 정규화는 표기 흔들림만 흡수하지 이름이 다른 건 못 잡는다.
...
**아직 지우면 안 되는 이유 (2026-08-12 대조):** `대웅징코샷`(214) · `루피어데포`(314) · `뮤코트라`(232)는 `quiz_answers.json`에 항목이 비어 있어 답이 legacy에만 있다. `시너지아`(144) · `리토바`(411)는 접미사 차이로 합쳐지지 않는 고아 키다. 이들이 다 빠진 뒤 삭제할 것.
